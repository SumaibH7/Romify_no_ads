package com.bluell.roomdecoration.interiordesign.ui.history.exterior
...
import com.bluell.roomdecoration.interiordesign.databinding.FragmentExteriorHistoryBinding
import com.bluell.roomdecoration.interiordesign.domain.repo.MyInteriorClicks
import com.bluell.roomdecoration.interiordesign.ui.history.HistoryAdapter
import com.google.gson.Gson
import dagger.hilt.android.AndroidEntryPoint
import kotlinx.coroutines.launch
import javax.inject.Inject
@AndroidEntryPoint
class ExteriorHistoryFragment : Fragment() {

    private var _binding:FragmentExteriorHistoryBinding ?= null
    private val binding get() = _binding!!

    private var currentPopupWindow: PopupWindow? = null

    private var arrayListMyCreations: MutableList<genericResponseModel>? = mutableListOf()
    private var myCreationAdapter: HistoryAdapter? = null

    private val viewModel: ExteriorHistoryViewModel by viewModels()
    ...
    override fun onCreateView(
        ...
        _binding = FragmentExteriorHistoryBinding.inflate(inflater,container,false)
        return binding.root
    }
    ...
    private fun initObservers() {
        appDatabase.genericResponseDao()?.getAllCreationsLive(0,Constants.EXTERIOR_END_POINT)?.observe(viewLifecycleOwner) {
            if (it.isNotEmpty()) {
    ...
    private fun initMyCreations() {
        ...
        myCreationAdapter = arrayListMyCreations?.let { HistoryAdapter(it,object: MyInteriorClicks {
            ...
            override fun onItemClick(item: genericResponseModel?, position: Int) {
                if (!item?.output.isNullOrEmpty() && item?.output!!.size>1){
                    Bundle().apply {
